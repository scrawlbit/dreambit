...
namespace DreamBit.Engine.Tests
{
    public class YSortIsoTests
    {
        ...
        public void YSort_DesenhaMaisAbaixoNaFrente()
        {
            ...
            Assert.True(order.IndexOf(back) < order.IndexOf(front), "quem tem Y menor desenha primeiro (atrás)");

            back.Transform.Position = new Vector2(0, 500); // agora atrás vira frente
            order = scene.VisibleInDrawOrder().ToList();
            Assert.True(order.IndexOf(front) < order.IndexOf(back), "reordena por Y ao mover");
        }
        ...
        public void Isometrico_MapeiaCelulaMundoEVolta()
        {
            ...
            Assert.Equal(32f, c.X, 0.001f);
            Assert.Equal(48f, c.Y, 0.001f);
        ...
        public void Serializacao_RoundTrip_YSortEOrientacao()
        {
            ...
            Assert.Equal(24f, e.Components.OfType<YSort>().Single().Offset);
            Assert.Equal(TileOrientation.Isometric, e.Components.OfType<TilemapRenderer>().Single().Orientation);
        }
    }
}
